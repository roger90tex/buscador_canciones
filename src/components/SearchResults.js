...
import { addSong } from '../redux/slice/librarySlice.js';
import {
  ResultsWrapper,
  ResultCard,
  SongInfo,
  ...
  ErrorWrapper,
  RetryButton,
} from  './styles/SearchResultsStyles.js';

const SearchResults = () => {
  const { results, error } = useSelector((state) => state.search);
  const dispatch = useDispatch();

  if (error) return <ErrorWrapper>Error: {error.toString()} <RetryButton onClick={() => window.location.reload()}>Reintentar</RetryButton></ErrorWrapper>;
